from gym.spaces import Box, MultiDiscrete, Discrete, Tuple as TupleSpace, Discrete
import logging
import numpy as np
import random
import time
...
@PublicAPI
class Unity3DEnvQMix(MultiAgentEnv):
    ...
    def __init__(
        ...
        """Initializes a Unity3DEnvQMix object.
        ...
        while True:
            ...
            worker_id_ = Unity3DEnvQMix._WORKER_ID if file_name else 0
            Unity3DEnvQMix._WORKER_ID += 1
            try:
        ...
        for behavior_name in self.unity_env.behavior_specs:
            ...
            if self.api_version[0] > 1 or (
                ...
                for agent_id in self.unity_env.get_steps(behavior_name)[0].agent_id:
                    key = behavior_name + "?agent_id={}".format(agent_id)
                    all_agents.append(key)
                    actions.append([action_dict[key]])
                if actions:
                    if actions[0][0].dtype == np.float32:
