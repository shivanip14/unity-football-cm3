from gym.spaces import Box, MultiDiscrete, Tuple as TupleSpace, Discrete
import logging
import numpy as np
import random
import time
...
class Unity3DEnv(MultiAgentEnv):
    ...
    def get_policy_configs_for_game(
        ...
        obs_spaces = {
            "SoccerTwos": TupleSpace(
                [
                    Box(-1.0, 1.0, (264,)),
                    Box(-1.0, 1.0, (72,)),
                ]
            ),
            "SoccerTwosRR": TupleSpace(
                [
                    Box(-1.0, 1.0, (264,)),
                    Box(-1.0, 1.0, (72,)),
                    Box(-1.0, 1.0, (2,)),
        ...
        action_spaces = {
            "SoccerTwos": MultiDiscrete([3, 3, 3]),
            "SoccerPlayerFlattenedActions": Discrete(27),
        }

        # Policies (Unity: "behaviors") and agent-to-policy mapping fns.
        if env_name == "SoccerTwos":
            policies = {
        ...
        elif env_name == "SoccerTwosFlattenedActions":
            policies = {
                "BluePlayer": PolicySpec(
                    observation_space=obs_spaces["SoccerTwos"],
                    action_space=action_spaces["SoccerTwosFlattenedActions"],
                ),
            }
